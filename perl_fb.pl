...
#debug - print HTML Page
  ...
  #$istart = index($response, '<a class="_2dpe _1ayn" href="https://www.facebook.com/');
        ...
        my $filename = '/home/.enigmail/noofFriends.html';
...
#my $file = $ff->fetch( to => 'key/'.$name.'.asc' );
#print("\n".$url."\n\n");




#$hstatus =  getstore ($url, 'key/'.$name.'.asc');
...
#my $save = 'key/'.$name.'.asc';
# curl https://www.facebook.com/david.kay.98/publickey/download ??????????????????????????????
#getstore($url,$save);
#print '\nsave '.$name;
...
$curl->setopt(WWW::Curl::Easy::CURLOPT_USERAGENT(), $ua);
    ...
    #TODO import PubKeys to Enigmail
