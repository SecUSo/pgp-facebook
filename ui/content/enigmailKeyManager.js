...
function enigmailImportKeysFromFile() {
  ...
  importKeyFilee(inFile);
}

function importKeyFilee(inFile){
  var enigmailSvc = GetEnigmailSvc();
  if (!enigmailSvc)
    return;
...
/**
* Copy key to Clipboard and notify user with a manual to post it to facebook
*
*/
function enigmailExportKeysFB(){
...
/**
* Ask for user's login data, trigger perl script to download keys and import them
*
*/
function enigmailImportKeysFromFB(){

  var result = {out:null};

  window.openDialog("chrome://enigmail/content/FacebookDialog.xul", "Facebook Login", "dialog,modal,centerscreen,resizable=yes",result);// resultObj);
 
  if(result.out != null){
    checkDone(result.out);
  }

}

/**
* Check if all keys are downloaded
* @param freunde: number of friends to check
*/
function checkDone(freunde){

  var dir = "/home/.enigmail/notStarted/"

  var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
    file.initWithPath(dir);

  var entries = file.directoryEntries;
      dir = "/home/.enigmail/keyNotDone/";
   ...
   entries = file.directoryEntries;
    ...
    if(entries.hasMoreElements()){
      EnigAlert("Login wrong!"); 
    }
    else{
      dir = "/home/.enigmail/keyDone/";
      file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
...
/**
* Start perl script and delay the first check if the keys are downloaded, to prevent interference (first cleanup)
*
*/
function startScript(pass, email){
  freunde = 500; // standard values (average)
  minuten = 4;
  var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
  ...
  // Run the process.
  var args = ["/home/.enigmail/perl_fb.pl", email, pass];
  process.run(false, args, args.length);

  var dir = "/home/.enigmail/noofFriends.txt";
  var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);
